<?php
/**
 * Basic class all modules should extend.
 */
class EselModule
{
    public $Esel;
    ...
    public static function isSafe($moduleName,$throw=1)
    {
        ...
        if ((!$hash) || (!file_exists(SL_CORE."hash/".$hash))) {
          if(!$throw){
            ...
            throw new Exception(EselLexicon::get("module_not_installed",array("moduleName"=>$moduleName)));
        }
    ...
    public static function getLexicon($entry,$phrase,$params=array(),$language=""){
      $tmp=explode("::",$entry);
      $module=$tmp[0];
      $topic="default";
      if(!empty($tmp[1])){
      ...
      return EselLexicon::get($phrase,$params,$language,$module,$topic);
    }
